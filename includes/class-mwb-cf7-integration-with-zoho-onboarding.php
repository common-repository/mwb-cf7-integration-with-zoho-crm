 ...
 */
if ( class_exists( 'Mwb_Cf7_Integration_With_Zoho_Onboarding' ) ) {
	return;
}
 ...
 */
class Mwb_Cf7_Integration_With_Zoho_Onboarding {
	...
	public function __construct() {
		...
		if ( defined( 'MWB_ZOHO_CF7_ONBOARD_PLUGIN_NAME' ) ) {
			self::$plugin_name = MWB_ZOHO_CF7_ONBOARD_PLUGIN_NAME;
		}
	...
	public function enqueue_styles() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_style( 'makewebbetter-onboarding-style', ZOHO_CF7_INTEGRATION_URL . 'admin/css/makewebbetter-onboarding-admin.min.css', array(), '1.0.0', 'all' );

			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_style( 'makewebbetter-onboarding-select2-style', ZOHO_CF7_INTEGRATION_URL . 'admin/css/select2.min.css', array(), '1.0.0', 'all' );
		}
	}
	...
	public function enqueue_scripts() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_script( 'makewebbetter-onboarding-scripts', ZOHO_CF7_INTEGRATION_URL . 'admin/js/makewebbetter-onboarding-admin.min.js', array( 'jquery' ), '1.0.0', true );
			...
			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_script( 'makewebbetter-onboarding-select2-script', ZOHO_CF7_INTEGRATION_URL . 'admin/js/select2.min.js', array( 'jquery' ), '1.0.0', false );
		}
	}
	...
	public function add_onboarding_popup_screen() {
		...
		if ( $this->is_valid_page_screen() && $this->can_show_onboarding_popup() ) {
			require_once ZOHO_CF7_INTEGRATION_DIRPATH . 'extra-templates/makewebbetter-onboarding-template-display.php';
		}
	}
	...
	public function add_deactivation_popup_screen() {
		...
		if ( ! empty( $pagenow ) && 'plugins.php' == $pagenow ) { // phpcs:ignore
			require_once ZOHO_CF7_INTEGRATION_DIRPATH . 'extra-templates/makewebbetter-deactivation-template-display.php';
		}
	}
	...
	public function add_on_boarding_form_fields() {
		...
		$fields = array(
			...
			wp_rand() => array(
				'id'          => 'monthly-revenue',
				'label'       => esc_html__( 'What is your monthly revenue?', 'mwb-cf7-integration-with-zoho' ),
				'type'        => 'radio',
			...
			wp_rand() => array(
				'id'          => 'industry_type',
				'label'       => esc_html__( 'What industry defines your business?', 'mwb-cf7-integration-with-zoho' ),
				'type'        => 'select',
				'name'        => 'industry_type_',
				'value'       => '',
				'multiple'    => 'yes',
			...
			wp_rand() => array(
				'id'          => 'onboard-email',
				'label'       => esc_html__( 'What is the best email address to contact you?', 'mwb-cf7-integration-with-zoho' ),
				'type'        => 'email',
				'name'        => 'email',
			...
			wp_rand() => array(
				'id'          => 'onboard-number',
				'label'       => esc_html__( 'What is your contact number?', 'mwb-cf7-integration-with-zoho' ),
				'type'        => 'text',
				'name'        => 'phone',
				'value'       => '',
				'required'    => 'yes',
	...
	private function hic_post( $endpoint, $post_params, $headers ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$status_code = 500;
			$_response   = esc_html__( 'Unexpected Error Occured', 'mwb-cf7-integration-with-zoho' );
			$errors      = $response;
